...
angular
  .module("desktopApp", [])
  .controller("indexController", function ($scope, $http, $interval, $window) {
    ...
    $scope.restart = function (configId) {
      console.log("restart configId:", configId);
    ...
    $scope.addTag = function (form) {
      ...
      if (form.unitsId) {
        ...
        // console.error("please assign unitId from pulse and reload OPCConnect by CTRL+R ")
        $window.alert(
        ...
        $http
          .post(
            properties.get("CONFIG_URL_PREFIX") +
              ...
              form.id +
              "/tags",
            (json = { dataTagId: form.newTag, ingestconfigId: form.id })
          )
          .then(function (response) {
            if (response.status == 200) {
    ...
    $interval(function () {
      ...
      $http
        .get(userApiUrl, { responseType: "arraybuffer" }) // Replace with the actual server URL
        .then(function (response) {
          ...
          var downloadSpeedMbps =
            (((response.data.byteLength / (requestTime / 1000)) * 8) /
              1000000) *
            10;
          ...
          var networkSpeedElement =
            document.getElementById("network-speed-mbps");
          ...
          networkSpeedElement.textContent =
            downloadSpeedMbps.toFixed(2) + " Mbps";
        })
        .catch(function (error) {
    ...
    $interval(function () {
      ...
      if (processes.length === 0) {
        ...
        const pids = processes.map(process => process.split(/\s+/)[1]).filter(pid => pid !== '');
        console.log('List of PIDs:', pids);  

        // if (pids.length === 0) {
        //   console.log(`No process found. Not killing.`);
        // }
      
        if (pids.length <= $scope.forms.length) {
          console.log(`Only ${pids.length} process found. Not killing.`);
        } else {
          const pidsToKill = pids.slice($scope.forms.length); 
          const killCommands = pidsToKill.map(pid => `taskkill /f /pid ${pid}`);
          ...
          killCommands.forEach((killCommand, index) => {
            exec(killCommand, (killError, killStdout, killStderr) => {
              if (killError) {
                console.error(`Error killing process with PID ${pidsToKill[index]}: ${killError}`);
                return;
              }
              ...
              console.log(`Process with PID ${pidsToKill[index]} killed successfully.`);
            });
          });
        }
      });
